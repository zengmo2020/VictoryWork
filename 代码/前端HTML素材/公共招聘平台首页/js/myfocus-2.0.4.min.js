	...
	myFocus.extend({
		...
		initCSS: function(p, a) {
			var b = [],
				...
				h = p.height || '';
			if(p.pic) {
				b.push('.' + p.__clsName + ' *{margin:0;padding:0;border:0;list-style:none;}.' + p.__clsName + '{position:relative;width:100%;height:100%;overflow:hidden;font:12px/1.5 Verdana;text-align:left;background:#fff;visibility:visible!important;}.' + p.__clsName + ' .pic{position:relative;width:100%;height:100%;overflow:hidden;}.' + p.__clsName + ' .txt li{width:' + w + 'px;height:' + p.txtHeight + 'px!important;overflow:hidden;}');
				if(p.wrap) p.$o.wrap('<div class="' + p.pattern + '_wrap"></div>');
				if(p.autoZoom) b.push('.' + p.__clsName + ' .pic li{text-align:center;width:100%;height:100%;}')
			}
			try {
